...
window.Aniways = {
  init: function(){
    if(Aniways.mapping !== null){
      this.aniwaysMapping = Aniways.parseMappings();
      this.decoder = new Decoder(this.mapping, this.assetsNamesToUrls);
    }
    if (document.getElementsByClassName('aniways-wall').length > 0){

      document.execCommand("enableObjectResizing", false, false);//firefox hack

      this.addWallObserver();

      $('.aniways-div').on('input', function(){
        if(Aniways.aniwaysMapping !== undefined){
          Aniways.highliter.highlight(this, Object.keys(Aniways.aniwaysMapping));
        }
      });
      ...
      $('body').on('mouseover', '.aniways-highlight' ,function(evt) {
        $(evt.currentTarget).popover('show');
        evt.stopPropagation();
      });
      $('body').on('touchend', '.aniways-highlight' ,function(evt) {
        $(evt.currentTarget).popover('show');
        evt.stopPropagation();
      });
  ...
  parseMappings: function(){
    var mappings = {};
    Object.keys(Aniways.mapping.phrasesToIcons).forEach(function (phraseName) {
      var iconPathsToReplaceTheEntirePhrase = Aniways.mapping.phrasesToIcons[phraseName][phraseName];
      if (iconPathsToReplaceTheEntirePhrase) {
        mappings[phraseName] = {};
        mappings[phraseName].iconPaths = iconPathsToReplaceTheEntirePhrase;
      }
    });
    return mappings;
  },
  mapping: JSON.parse(localStorage.getItem('aniwaysMappings')),
  ...
  addWallObserver: function(callback) {
    ...
    function handleNode(node, callback){
      if(typeof node.getElementsByClassName === 'function'){
        ...
        if(message.length > 0){
          var decodedMessage = Aniways.decoder.decodeMessage(message[0].innerHTML);
  ...
  setMapping:function(){
    if (this.readyState === 4) {
      if (this.status === 200) {
        Aniways.mapping = JSON.parse(this.responseText);
        Aniways.aniwaysMapping = Aniways.parseMappings();
        localStorage.setItem("aniwaysMappings", JSON.stringify(Aniways.mapping));
      } else {
  ...
  resizeImage: function (image){
    ...
    var w = $(image).width();
    var h = $(image).height();
    var textAreaLinehight = $(".aniways-div").css("line-height").replace("px", "");
    var unicodeWidth = 14;//Unicode actuall width?
    return Aniways.scaleImageSimple(image, unicodeWidth, textAreaLinehight);
  },

  scaleImageSimple: function (image, targetwidth, targetheight){
  ...
  encodeText: function(){
    var root = document.getElementsByClassName('aniways-div')[0];
    var ret = recEncodeText(root);
    root.innerHTML = "";
    console.log(ret);
    return ret;
    ...
    function encodeImage(imageNode){
      ...
      var id = Object.keys(Aniways.mapping.iconIdsToNames).filter(function(key) {return Aniways.mapping.iconIdsToNames[key] === imageName;})[0];
  ...
  generatePopupHtml: function (spanNode){
    var assets = "";
    var phrase = spanNode.data('phrase');
    var imageCount = Aniways.aniwaysMapping[phrase].iconPaths.length;
    for (var i=0; i< imageCount; i++){
      assets += "<img class='aniways-popover-image' onClick=Aniways.replaceWithImage(event) src='http://az493648.vo.msecnd.net/aniways-assets/android/ldpi/" + Aniways.aniwaysMapping[phrase].iconPaths[i] + "' data-phrase=" + phrase + "></img>";
    }
...
//var jQuery, jquery_version = '1.10.2';
//aniways_mapping= {};
//
...
function Decoder(phraseMapping, assetsToUrls){
  ...
  function unicodeDecoding(message){
    ...
    for (var i=0; i<messageEncodingData.data.length; i++ ) {
      ...
      if(imagePath === undefined){
        ...
        html += "<img class='aniways-wall-image' src='" + imagePath + "'  title='" +
...
Aniways.highliter = (function(){
  function highlightNode (node, re) {
    ...
    function addPopover(highlighter){
      $.each(highlighter.find('.aniways-highlight'), function(index, span){
        span = $(span);
        span.attr('data-phrase', span.text().toLowerCase());
        span.attr('onclick', 'void(0)');
        $(span).popover({selector:'.aniways-highlight', html: true, placement: 'top', content: Aniways.generatePopupHtml($(span)), trigger: 'manual'});
      });
    }
  }
